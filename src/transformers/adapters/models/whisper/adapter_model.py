...
class WhisperAdapterModel(EmbeddingAdaptersWrapperMixin, ModelWithFlexibleHeadsAdaptersMixin, WhisperPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs
        ...
        if input_features is not None and x.shape[1] == input_features.shape[1]:
            eos_mask = input_features.eq(self.config.eos_token_id)
...
class BartModelWithHeads(WhisperAdapterModel):
